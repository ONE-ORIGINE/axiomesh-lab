...
Architecture:
  ...
  │  Receives: ContextEnvelope with drone drone_state + harmony  │
  ├────────────────────────────────────────────────────────┤
...
@dataclass
class DroneState:
    """Physical drone_state of one drone — updated from sensors via SAVOIR."""
    ...
    def update_from_savoir(self, savoir: Savoir, drone_id: str):
        """Sync drone_state from SAVOIR knowledge base."""
...
class Drone(Element):
    ...
    def sensor_update(self, pos: Tuple[float,float,float],
        ...
        """
        Update drone_state from physical sensors — this is KNOWN, not estimated.
        The drone KNOWS its position from GPS+IMU.
        """
        sid = self.drone_id
        self.drone_state.x, self.drone_state.y, self.drone_state.z = pos
        self.drone_state.battery_pct = battery
    ...
    async def on_impacted(self, reaction: Reaction, frame: Dict):
        ...
        if at == "drone.move" and reaction.is_success:
            ...
            # Update DroneState
            self.drone_state.x += float(payload.get("dx", 0))
            self.drone_state.y += float(payload.get("dy", 0))
            self.drone_state.z += float(payload.get("dz", 0))
            self.drone_state.battery_pct -= 0.5
            self.set_dynamic("battery", self.drone_state.battery_pct)

        elif at == "drone.takeoff" and reaction.is_success:
            self.drone_state.z  = float(payload.get("altitude", 5.0))
            self.drone_state.armed    = True
            self.drone_state.airborne = True
            self.savoir.assert_known(f"{sid}.pos_z",   self.drone_state.z, "fc")
            ...
            self.drone_state.z        = 0.0
            self.drone_state.airborne = False
...
class DroneSwarmEnv(Environment):
    ...
    def _build_contexts(self):
        ...
        def battery_ok(ctx, frame):
            ...
            return d.drone_state.battery_pct > 15.0 if d else True

        def gps_ok(ctx, frame):
            did = frame.get("actor_id","")
            d = self.drones.get(did)
            return d.drone_state.gps_lock if d else True

        def is_airborne(ctx, frame):
            did = frame.get("actor_id","")
            d = self.drones.get(did)
            return d.drone_state.airborne if d else False
    ...
    def _register_flight_actions(self):
        async def h_takeoff(actor, payload, ctx, frame):
            ...
            if d.drone_state.airborne: return Reaction.reject("drone.takeoff","Already airborne")
            d.drone_state.mode = "GUIDED"
        ...
        async def h_move(actor, payload, ctx, frame):
            ...
            prev = SenseVector.spatial(f"pos@{d.drone_state.x:.1f},{d.drone_state.y:.1f}", 0.5)
            npos = SenseVector.spatial(f"pos@{d.drone_state.x+dx:.1f},{d.drone_state.y+dy:.1f}", 0.95)
        ...
        async def h_hover(actor, payload, ctx, frame):
            ...
            return Reaction.ok("drone.hover",
                f"{d.name} holding position at z={d.drone_state.z:.1f}m",
                result={"position":d.drone_state.position},
    ...
    def _register_navigation_actions(self):
        async def h_goto(actor, payload, ctx, frame):
            ...
            tz=float(payload.get("z", d.drone_state.z))
            dist=math.sqrt((tx-d.drone_state.x)**2+(ty-d.drone_state.y)**2+(tz-d.drone_state.z)**2)
    ...
    def _register_scan_actions(self):
        ...
        async def h_photo(actor, payload, ctx, frame):
            ...
            return Reaction.ok("drone.photo",
                f"{d.name} capturing at ({d.drone_state.x:.1f},{d.drone_state.y:.1f},{d.drone_state.z:.1f})",
                result={"position":d.drone_state.position},
    ...
    def _register_preflight_actions(self):
        ...
        async def h_preflight_check(actor, payload, ctx, frame):
            ...
            checks = {
                "battery": d.drone_state.battery_pct > 50,
                "gps":     d.drone_state.gps_lock,
...
class SwarmSavoir:
    ...
    def __init__(self, drones: List[Drone]):
        self.drones  = {d.drone_id: d for d in drones}
        # Shared drone_state matrix: all drones × [pos_x, pos_y, pos_z, battery]
    ...
    def inter_drone_distance(self, id_a: str, id_b: str) -> Optional[float]:
        ...
        dx = a.drone_state.x - b.drone_state.x; dy = a.drone_state.y - b.drone_state.y
        dz = a.drone_state.z - b.drone_state.z
        return math.sqrt(dx**2+dy**2+dz**2)

    def env_embedding(self) -> List[float]:
        """
        φ_swarm = flatten(M_Σ) — the swarm drone_state as a flat embedding vector.
        This is the persistent numerical memory of swarm drone_state.
    ...
    def savoir_summary(self) -> str:
        ...
        lines = ["SWARM SAVOIR:", ""]
        for did, drone in self.drones.items():
            pos = f"({drone.drone_state.x:.1f},{drone.drone_state.y:.1f},{drone.drone_state.z:.1f})"
...
async def drone_demo():
    ...
    for dx,dy,dz,label in moves:
        ...
        print(f"  Position after {label}: "
              f"({alpha.drone_state.x:.1f},{alpha.drone_state.y:.1f},{alpha.drone_state.z:.1f})"
    ...
    alpha.sensor_update((alpha.drone_state.x,alpha.drone_state.y,alpha.drone_state.z),
    ...
    print(f"\n  This vector IS the persistent numerical memory of swarm drone_state.")
